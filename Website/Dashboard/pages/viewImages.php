...
?>
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">View Your Images</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
			...
			if ($result->num_rows > 0) {
			    ...
			    while($row = $result->fetch_assoc()) {
			        $esid = $row['ESID'];
			    	$sql1 = "SELECT * FROM ESInfo WHERE ESID ='$esid'";
		        	$result1 = $conn->query($sql1);
		
				if ($result1->num_rows > 0) {
				    // output data of each row
				    while($row1 = $result1->fetch_assoc()) {
					$plantID = $row1["PlantID"];				
				}}
			      $timestamp = $row['date'];
			      echo '<div class="col-lg-2"><img src="' . $row["name"] . '" width="200" height="200"><center><i> ' . $row['des'] . ' </i></br><b>ESID: </b> '. $row["ESID"] .' </br><b>Date Added: </b> ' . $timestamp  . ' </center> </div>';
			}}
      		?>
      </div></div></div>
